...
1. **Biome** - Primary linter and formatter (replaces ESLint/Prettier)
   ...
   - Auto-imports organization enabled on save

2. **Lefthook** - Git hooks automation
   - Configuration: `lefthook.yml`
   - Pre-commit: Runs `pnpm lint`, `pnpm format`, and `pnpm check` (all in parallel)
...
- Pre-commit hooks automatically run Biome checks and TypeScript type checking
- Use `pnpm` (not `npm` or `yarn`) as package manager
- Lefthook is only built for `pnpm` (see `package.json` > `pnpm.onlyBuiltDependencies`)
...
- Include appropriate scope identifiers
